...
<div class="main-body">
    ...
    <div class="main-content">
        <div class="main-content-div">
            <p class="main-content-div-index"><c:choose><c:when test="${order.orderZt=='10'}">风控审核</c:when><c:when test="${order.orderZt=='11'}">出履约险</c:when><c:otherwise>查看详情</c:otherwise></c:choose><span class="page-error">${errorMsg}</span></p>
            <form:form  action="${ctx}/riskAudit" method="post" modelAttribute="orderOperate">
            ...
            <div class="form-group">
                <div class="row">
                    ...
                    <%--<div class="col-md-4">--%>
                        <%--<label for="loan" >打款总额</label><span class="static-error">*</span><form:errors path="loan" cssClass="form-error"></form:errors>--%>
                        <%--<input name="loan" id="loan" type="number" class="form-control" value="${(orderOperate!=null)?orderOperate.loan:'0'}" type="text"  <c:if test="${!(order.orderZt=='10'|| order.orderZt=='11')}"> readonly="readonly"</c:if> required>--%>
                    <%--</div>--%>
                </div>
            </div>
            ...
            <c:forEach items='${orderPicList}' var="obj">
                <c:if test="${obj.orderZt=='10'||obj.orderZt=='11'}">
                    <div class="form-group">
                        <label >${obj.pic.picName}</label>
                        <c:if test="${(order.orderZt=='10'|| order.orderZt=='11')}">
                        <input type="file" name="${obj.pic.picID}" id="${obj.pic.picID}" accept="image/png,image/jpeg" <c:if test="${!(order.orderZt=='10'||order.orderZt=='11')}">disabled="disabled"</c:if>>
            ...
            <c:if test="${order.orderZt=='10'||order.orderZt=='11'}">
                <shiro:hasPermission name="rest[riskAudit:post]">
