...
<div class="main-body">
    ...
    <div class="main-content">
        <div class="main-content-div">
            <p class="main-content-div-index"><c:choose><c:when test="${order.orderZt=='01'}">确认收款</c:when><c:otherwise>收款详情</c:otherwise></c:choose><span class="page-error">${errorMsg}</span></p>
            <form:form  action="${ctx}/confirmAudit" method="post" modelAttribute="orderOperate">
                ...
                <div class="form-group">
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type=text t_value="" o_value=""  name="loan" id="loan" value="${orderOperate.loan!=null?orderOperate.loan:'0'}" <c:if test="${order.orderZt!='01'}"> readonly="readonly"</c:if> required class="form-control"  onkeypress="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value" onkeyup="if(!this.value.match(/^[\+\-]?\d*?\.?\d*?$/))this.value=this.t_value;else this.t_value=this.value;if(this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?)?$/))this.o_value=this.value" onblur="if(!this.value.match(/^(?:[\+\-]?\d+(?:\.\d+)?|\.\d*?)?$/))this.value=this.o_value;else{if(this.value.match(/^\.\d+$/))this.value=0+this.value;if(this.value.match(/^\.$/))this.value=0;this.o_value=this.value}">
                        </div>
                    </div>
                </div>
                ...
                <c:forEach items='${orderPicList}' var="obj">
                    <c:if test="${obj.orderZt=='01'}">
                        <div class="form-group">
                            <label>${obj.pic.picName}</label>
                            <c:if test="${order.orderZt=='01'}">
                            <input type="file" name="${obj.pic.picID}" id="${obj.pic.picID}" accept="image/png,image/jpeg" <c:if test="${order.orderZt!='01'}">disabled="disabled"</c:if>>
                ...
                <c:if test="${order.orderZt=='01'}">
                    <shiro:hasPermission name="rest[confirmAudit:post]">
